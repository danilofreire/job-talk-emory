...
# Hello, everyone! <br> Nice to see you all! 😉 {background-color="#2d4563"}
...
- Writing a function is like writing a sentence: [it requires some structure!]{.alert} 🤓📚
...
- If you use [keyword arguments]{.alert}, you can pass arguments by name in any order 😉
...
## Let's create a function together 🛠️ {#sec:rectangle}
...
- [What went wrong?]{.alert}
  ...
  - I told you that [order matters]{.alert}! 😂
:::
...
# Lambda (anonymous) functions 🕵🏽‍♀️ {background-color="#2d4563"}
...
- ️[Lambda functions]{.alert} are perfect for this!
...
## Try it yourself! 🛠️ {#sec:exercise02}
...
# Great job, everyone! 🎉 {background-color="#2d4563"}

# Global vs local variables 🌍 {background-color="#2d4563"}
...
- Scope means: [Where in your code can you use a variable?]{.alert} 🤔
...
# Best practices and pitfalls 🚧 {background-color="#2d4563"}
...
# But wait, there's more! 😅 {background-color="#2d4563"}
...
# ...and we're back! 😄 {background-color="#2d4563"}
...
# Thanks again! 😃 {background-color="#2d4563"}
...
## Appendix 08: Error handling {#sec:appendix08}

:::{style="margin-top: 30px; font-size: 19px;"}
:::{.columns}
:::{.column width="60%"}
- [Error handling]{.alert} is a way to [manage errors]{.alert} that may occur during the execution of your code
- It helps you [handle unexpected situations]{.alert} gracefully
...
:::{.column width="40%"}
:::{style="font-size: 21px;"}
-  `except IndexError:` [catches errors when the index is invalid]{.alert} for the list!
-  Without `try...except`, your program would [crash with an `IndexError`]{.alert} if you tried to access an invalid index.
-  Think of it as a [safety net]{.alert .alert-info} for your code!

-  This is especially useful when:
    -  Dealing with [user input]{.alert} (which might be unpredictable)
    -  Working with [external data]{.alert} (which might have unexpected formats)
:::
:::
:::
:::

## Appendix 09: Type checking {#sec:appendix09}

:::{style="margin-top: 30px; font-size: 24px;"}
- [Type checking]{.alert} is a way to [ensure that the arguments passed to a function are of the expected type]{.alert}
- It helps you [catch errors early]{.alert} and [improve code readability]{.alert}
...
- This ensures that `a` and `b` are either integers or floats 😉
- If not, it raises a `TypeError` with a custom error message
- [This is a good practice]{.alert} to [avoid unexpected errors]{.alert} later on
:::
